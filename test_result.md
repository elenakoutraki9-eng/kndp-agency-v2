...
user_problem_statement: "Verify Hero section stats row removal on the KNDP agency site. The stats row (previously showing '6 Projects on the bench', '2h Response time promise', '4w Typical website delivery' with data-testid='hero-stat-*') has been REMOVED from the Hero section. Verify at 1440x800 desktop and 390x844 mobile: 1) Stats row/counters are completely gone. 2) Headline, subheading, and two CTA buttons display correctly with no big empty gap where stats used to be. 3) Phone mockup, rotating chip, and floating card still render correctly. 4) No console errors. 5) Quick regression: services, portfolio carousel, and contact form still work."
